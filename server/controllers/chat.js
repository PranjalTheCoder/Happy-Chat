...
const newGroupChat = TryCatch(async(req, res, next) =>{
    // hume group chat bnane ke liye do cheeze chahiye 
    // name aur members bina members ke kaddu bnauga group chat
      ...
      //ye vala emit sbhi ko krna including me 
      emitEvent(req, ALERT, allMembers, `Welcome to ${name} Group`);
      //ye vala emit sbhi ko krna excluding me 
      //jese he me group create kru to baki users to kese naviagte kru
      //jab group create ho to REFETCH ho jaye aur users ki chat bhi
...
//agar ham user ke friends ki array bna lete to iski need nhi hoti
  
   const chats = await Chat.find({members: req.user })
   .populate(
    "members","name avatar"
   );
   //yha par hum aggregation pipeline use kr skte hai
    // we perform multiple operations using aggregation pipeline 
    // [(), (), (), ()] left to right funtions execute
    // but we are not use here
...
const getChatDetails = TryCatch(async (req, res, next) => {
  if (req.query.populate === "true") {
    const chat = await Chat.findById(req.params.id)
      .populate("members", "name avatar")
      .lean();
    // lean krne se ab ye chat member database ka obj nhi h 
    // ye classsy js ka on=bj hai ab changes kr skte h
    if (!chat) return next(new ErrorHandler("Chat not found", 404));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  // agar group chat and creator ho tb bhi tm delte nhi kr skte 
  ...
  //mongodb big topics: indexing, aggregation, data sharding
...
export {
  addMembers, getMyChats,
  getMyGroups, leaveGroup, newGroupChat, removeMember, sendAttachments,
